...
# INSTALLATION
We recommend users to download the EvoEF source-code package to your computer and build the EvoEF executable on your own. After downloading and unzipping the package, change into the $path/EvoEF/ directory and run " <i> g++ -O3 --fast-math -o EvoEF src/*.cpp </i>" or "<i> ./build.sh </i>" if you are working in a UNIX/Linux environment. If you are working on a Windows system, you need to install the g++ compiler first.
